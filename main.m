...
% Flapwise frequency
[~, flap_locs] = findpeaks(x(1, transient_idx:end), t(transient_idx:end));
if numel(flap_locs) > 1
    flap_periods = diff(flap_locs);
    flap_freq = 1/mean(flap_periods);
else
    flap_freq = NaN;
end

% Edgewise frequency
[~, edge_locs] = findpeaks(x(2, transient_idx:end), t(transient_idx:end));
if numel(edge_locs) > 1
    edge_periods = diff(edge_locs);
    edge_freq = 1/mean(edge_periods);
else
    edge_freq = NaN;
end

% Compare with natural frequencies
fprintf('Flapwise: Simulated freq = %.3f Hz, Natural freq = %.3f Hz\n', flap_freq, omega_1flap/(2*pi));
fprintf('Edgewise: Simulated freq = %.3f Hz, Natural freq = %.3f Hz\n', edge_freq, omega_1edge/(2*pi));
