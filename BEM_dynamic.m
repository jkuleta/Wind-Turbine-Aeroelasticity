function [Rx, FN, FT, P, a, a_prime] = BEM_dynamic( ...
...
for i=1:NBS
    ...
    if dynamic_inflow
        ...
        a_nw = a_prev * exp(-dt / tau_nw) + ax * (1 - exp(-dt / tau_nw));
        a_fw = a_prev * exp(-dt / tau_fw) + ax * (1 - exp(-dt / tau_fw));
        ax_dyn = 0.6 * a_nw + 0.4 * a_fw;

        ap_nw = a_prime_prev * exp(-dt / tau_nw) + ax_prime * (1 - exp(-dt / tau_nw));
        ap_fw = a_prime_prev * exp(-dt / tau_fw) + ax_prime * (1 - exp(-dt / tau_fw));
